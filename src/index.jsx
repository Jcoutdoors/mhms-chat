...
const APP_CONFIG = {
  ...
  // Consultation data now lives in the organization config seam (MHMS_ORG_CONFIG.consult,
  // Stage 2 Slice 3) so it has one organization-level source of truth.
      ...
      <div style={{ maxWidth: 560, margin: '0 auto' }}>
        ...
        <div style={{ borderRadius: 16, padding: '22px 24px', marginBottom: 28, background: 'linear-gradient(135deg, #3a55d9 0%, #2f44b8 100%)', boxShadow: '0 12px 30px rgba(47,68,184,0.28)', color: '#fff' }}>
          ...
          <p style={{ fontSize: 13.5, lineHeight: 1.6, color: 'rgba(255,255,255,0.92)', marginBottom: 14 }}>
            Mark hosts a live Zoom consultation every other week at <strong>{MHMS_ORG_CONFIG.consult.time}</strong>. Same link every time, so you can join right from here.
          </p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginBottom: 18 }}>
            {MHMS_ORG_CONFIG.consult.dates.map(d => (
...
function CommunityDestination({ runtime, currentUser, isMobile, mobileNavOpen, setMobileNavOpen, rosterMembers, onEditProfile, onLogout, onSelectChannel, onThreadNoteClick, welcome, welcomeBack }) {
  ...
  if (!chatClient || Object.keys(channelMap).length === 0) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', minHeight: 0, background: 'var(--anchor-canvas, #fff)' }}>
  ...
  return (
    <div style={{ display: 'flex', height: '100%', minHeight: 0, fontFamily: "var(--anchor-font-body, 'DM Sans', sans-serif)", background: 'var(--anchor-canvas, #e7e9f1)', padding: isMobile ? 0 : 14, overflow: 'hidden' }}>
...
createRoot(document.getElementById('root')).render(
  <ThemeProvider orgAccent={MHMS_ORG_CONFIG.brandColor}>
    <App />
  </ThemeProvider>
);
